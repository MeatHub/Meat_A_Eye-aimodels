"""
돼지 부위 전체 테스트 데이터셋 통합 평가 스크립트
pork_dataset_1~N/test 폴더를 모두 합쳐서 한 번에 테스트.
"""
import torch
...
RESULT_DIR = r"C:\Pyg\Projects\meathub\Meat_A_Eye-aimodels\pork_test_results_all"

# 돼지 7부위
CLASS_NAMES = ['Pork_Belly', 'Pork_Ham', 'Pork_Loin', 'Pork_Neck',
...
def find_test_dirs(data_root):
    """pork_dataset_*/test 폴더를 자동 탐색."""
    dirs = []
    for entry in sorted(os.listdir(data_root)):
        test_dir = os.path.join(data_root, entry, "test")
        if entry.startswith("pork_dataset_") and os.path.isdir(test_dir):
            dirs.append((entry, test_dir))
    return dirs


def collect_images(test_dir, dataset_name):
    """한 test 폴더에서 돼지 이미지 수집. (경로, 정답, 출처 데이터셋)"""
    images = []
    for class_name in CLASS_NAMES:
        ...
        for ext in ['*.jpg', '*.jpeg', '*.png', '*.bmp', '*.webp']:
            for img_path in glob.glob(os.path.join(class_dir, ext)):
                images.append((img_path, class_name, dataset_name))
    return images
...
def run_all_tests():
    ...
    if not test_dirs:
        print("❌ pork_dataset_*/test 폴더를 찾을 수 없습니다.")
        return

    print(f"\n{'='*100}")
    print(f"🐷 돼지 부위 통합 테스트 — 모델: {os.path.basename(MODEL_PATH)}")
    print(f"{'='*100}")
    print(f"  발견된 테스트 폴더:")
    ...
    # ── 추론 ──
    print("-" * 110)
    print(f"{'데이터셋':<18} | {'파일명':<30} | {'정답':<22} | {'예측':<22} | {'신뢰도':<8} | {'결과'}")
    print("-" * 110)
    ...
    for img_path, ground_truth, ds_name in all_images:
        ...
        if SAVE_REPORTS:
            ...
            cv2.putText(bar, txt, (10, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
        ...
        print(f"{ds_name:<18} | {filename[:30]:<30} | {ground_truth:<22} | {pred_label:<22} | {confidence*100:>6.1f}% | {mark}")
    ...
    for name, _ in test_dirs:
        ...
        print(f"\n  ── {name} (정확도: {ds_acc:.1f}%) ──")
        print(f"  {'클래스':<25} | {'맞춤':>5} | {'전체':>5} | {'정확도':>8} | {'주요 오분류'}")
        for cn in CLASS_NAMES:
            s = ds["class_stats"][cn]
            ...
            print(f"  {cn:<25} | {s['correct']:>5} | {s['total']:>5} | {a:>5.1f}% {bar} | {ws}")

    # 3) 전체 클래스별 통합 정확도
    print(f"\n{'='*100}")
    print("📊 [전체 통합 — 클래스별 정확도]")
    print("=" * 100)
    print(f"{'클래스':<25} | {'맞춤':>6} | {'전체':>6} | {'정확도':>10} | {'주요 오분류'}")
    print("-" * 100)
    ...
    for cn in CLASS_NAMES:
        ...
        bar = "█" * int(acc // 10) + "░" * (10 - int(acc // 10))
        print(f"{cn:<25} | {s['correct']:>6} | {s['total']:>6} | {acc:>6.1f}% {bar} | {ws}")
